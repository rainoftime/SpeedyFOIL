...
/*    ---------------  */
{
	...
	/*
	 if (0 == strcmp("arpReqCtl", R->Name)) {
	 printf("\nEvaluate Relation %s LitBits=%lf ...\n", R->Name, LitBits);
	...
	ForEach(i, 0, NDuplicateVars-1)
	{
		...
		W = OldVar[i];
		if (W > Current.MaxVar)
			W = A[W - Current.MaxVar]; /* special */
		if (NowTPos || !DifferOnNegs[i]) {
			Verbose(3)
				printf(" %s%s%s", Variable[W]->Name,
						(DifferOnNegs[i] ? "=+" : "="), Variable[V]->Name);
		}
	...
	if (NWeakLits >= MAXWEAKLITS && !Determinate && (!PossibleT || WeakT)
			&& (!PossibleF || WeakF)) {
		Verbose(3)
			printf(" (weak)\n");
		return;
	...
	if (Determinate && Gain < DETERMINATE * MaxPossibleGain
	...
	if (PosGain > 1E-3
		...
		/*
		 printf("propose true literal: %s, NowTTot=%d LitBits=%lf OrigTPos=%d OrigTTot=%d PosGain=%lf WeakT=%d\n",
		 R->Name, NowTTot, LitBits, OrigTPos, OrigTTot, PosGain, WeakT);
		 */
		ProposeLiteral(R, true, A, NowTTot, LitBits, OrigTPos, OrigTTot,
				PosGain, WeakT);
	}
	...
	if (NegGain > 1E-3
		...
		/*
		 printf("propose false literal: %s, NowFTot=%d LitBits=%lf OrigFPos=%d OrigFTot=%d NegGain=%lf WeakF=%d\n",
		 R->Name, NowFTot, LitBits, OrigFPos, OrigFTot, NegGain, WeakF);
		 printf("propose false literal: %s\n\n", R->Name);
		 */
...
/*    --------------  */
{
	...
	ForEach(i, 0, NConstVars-2)
	{
		...
		ForEach(j, i+1, NConstVars-1)
		{
			...
			if (!Compatible[Variable[V]->Type][R->Type[W]])
				continue;
	...
	PossibleT = true;
	PossibleF = NegatedLiteralOK = (NEGLITERALS || R == GTVAR|| R == GTCONST ||
	...
	MinUsefulGain =
			NPossible < MAXPOSSLIT ?
					MINALTFRAC * BestLitGain :
	...
	if (NLit < 5) {
		...
		while ( Encode(MinSaveableCover) <= NewClauseBits)
			MinSaveableCover++;
	} else {
...
/*       -------------  */
{
	...
	for (TSP = Current.Tuples; (Case = *TSP++);) {
		if (FirstNegTuple && !Positive(Case)) {
			...
			NewTNegThresh = (
					NNewVars && BestLitGain < 1E-2 ?
							MAXTUPLES : NegThresh(NowTPos, NewTPos));
		...
		if (MissingValue(R, A, Case)) {
			...
			TermTest(PossibleT, NewTTot > MAXTUPLES);
			...
			if (Positive(Case)) {
				...
				TermTest(PossibleT, (NewTTot - NewTPos) > NewTNegThresh + 1E-3);
			}
			...
			if (Positive(Case)) {
				...
				TermTest(PossibleF, NowFNeg > NowFNegThresh + 1E-3);
			}
...
/*    -------------  */
{
	...
	RemainingNeg = (Current.NTot - Current.NPos) - (NowMTot - NowMPos)
			- (NowTTot - NowTPos) - NowFNeg;

	for (TSP = Current.Tuples + (NowMTot + NowTTot + NowFPos + NowFNeg); (Case =
			*TSP++);) {
		...
		if (NowFNeg + RemainingNeg <= MaxFNeg)
			break;
	}
}
...
/*    ------------  */
{
	...
	for (i = 0; i < NDuplicateVars; i++) {
		if (!Positive(Case) && DifferOnNegs[i])
			continue;
		...
		for (j = 0; j < NFound; j++) {
			if (Found[j][Col]
					== (P <= Current.MaxVar ?
							OldVarVal : Found[j][P - Current.MaxVar])) {
				continue;
			}
	...
	if (NConstVars) {
		...
		ForEach(i, 0, NConstVars-1)
		{
			...
			for (j = 0; j < NFound; j++) {
				if (FirstBinding[Col] == Found[j][Col])
					continue;
...
/*     ---------  */
{
	return P <= 0 ?
			0.0 :
			SAMPLE * (P1 + 1)
					* (exp(LN2 * (Current.BaseInfo - MinUsefulGain / P)) - 1);
}
...
/*       ----------       */
{
	...
	ForEach(i, 1, R->Arity)
	{
		...
		if (V <= Current.MaxVar && Unknown(V, T))
			return true;
	}
...
/*    -------------  */
{
	...
	if (BestGain >= 0) {
		...
		NowFNeg = (Current.NTot - Current.NPos) - (NowTTot - NowTPos)
				- (NowMTot - NowMPos);
...
/*    -----------  */
{
	...
	FGain = Worth(Current.NPos - NowTPos - NowMPos,
			Current.NPos - NowTPos - NowMPos, Current.NTot - NowTTot - NowMTot,
			0);
