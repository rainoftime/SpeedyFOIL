...
Boolean Check_UserConstraints(Relation R) {
	int i = 0;
	Boolean result = false; 
	for (i = 0; i < UserInfo.num; i++) {
		result = result || (!strcmp(Target->Name,UserInfo.head[i]) && !strcmp(R->Name,UserInfo.body[i]));
	}
	return result;
	//return (
	//		false 
		 //	|| (!strcmp(Target->Name,"path") && !strcmp(R->Name, "edge")) 
	//	    || (!strcmp(Target->Name,"Copy") && !strcmp(R->Name, "Load")) 
		 //   || (!strcmp(Target->Name,"Ok") && !strcmp(R->Name, "Err")) 
		 //   || (!strcmp(Target->Name,"reachableCast") && !strcmp(R->Name, "unsafeDowncast")) 
		 //   || (!strcmp(Target->Name,"ptsVH") && !strcmp(R->Name, "ptsVT")) 
		 //   || (!strcmp(Target->Name,"opSucc") && !strcmp(R->Name, "Err")) 
		 //   || (!strcmp(Target->Name,"opSucc") && !strcmp(R->Name, "Ok")) 
		 //   || (!strcmp(Target->Name,"opSucc") && !strcmp(R->Name, "opSucc0" && (NLit == 2))) 
         //   || (!strcmp(Target->Name,"Ok") && !strcmp(R->Name, "opSucc") && (NLit <= 1) )  
         //   || (!strcmp(Target->Name,"Ok") && !strcmp(R->Name, "opSucc0") && (NLit <= 1))  
         //   || (!strcmp(Target->Name,"Ok") && !strcmp(R->Name, "fnRetV") && (NLit <= 1))  
         //   || (!strcmp(Target->Name,"Err") && !strcmp(R->Name, "Ok") && (NLit <= 2)) 
         //   || (!strcmp(Target->Name,"Err") && !strcmp(R->Name, "opSucc"))  
         //   || (!strcmp(Target->Name,"Err") && !strcmp(R->Name, "opSucc0") && (NLit <= 0)) 
         // || (!strcmp(Target->Name,"Err") && !strcmp(R->Name, "fnCallRet"))  
	//	);
}
...
/*    ---------------  */
{
	...
	{
		R = RelnOrder[i];
		// printf("** Target : %s \t R : %s \t NLit : %d \n",Target->Name, R->Name, NLit);

		if (USERCUSTOM && Check_UserConstraints(R))
		  {
		//		printf("here!!!\n");
				continue;
		  }
...
/*    -------------                */
{
	...
	if ( SavedClause &&
			( SavedClauseAccuracy > Accuracy ||
			  (SavedClauseAccuracy == Accuracy && SavedClauseCover > Current.NOrigPos) ||
			  (SavedClauseAccuracy == Accuracy &&
