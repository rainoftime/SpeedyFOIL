	...
	if (R->Neg) {
		...
		if (WorldSize <= 10 * MAXTUPLES) {
			...
			while (NegTuples && (Case = NextConstTuple(R, Case))) {
				if (!Join(R->Pos, R->PosIndex, DefaultVars, Case, R->Arity,
						true) && drand48() <= NegTuples / (float) Remaining--) {
...
/*    ------------  */
{
	...
	for (TSP = Current.Tuples; (Case = *TSP++);) {
		...
		if (!BuiltIn && RSign) {
			...
			memcpy(New.Tuples[New.NTot], Case,
					(New.MaxVar + 1) * sizeof(Const));
			New.NTot++;
			if (Positive(Case))
				New.NPos++;
		}
...
/*      --------  */
{
	return (n < 1000 ?
			LogFact[n] :
			(n + 0.5) * Log2(n) - n * Log2e + Log2sqrt2Pi
					- (n * 0.7623) / 820000);
}
