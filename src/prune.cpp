...
/*    --------------  */
{
	...
	if (QuickPrune(NewClause, Target->Arity, Used)) {
		if (SatisfactoryNewClause(Errs)) {
			Verbose(3)
				printf("\tAccepting quickpruned clause\n");
			Changed = true;
		} else {
...
/*       -------------  */
{
	...
	for (TSP = Current.Tuples; (Case = *TSP++);) {
		if ( Positive(Case) && (Unknown(V, Case) || Case[V] != Case[W]))
			return false;
	}
...
/*    ------  */
{
	...
	if (V > Target->Arity) {
		...
		{
			L = NewClause[i - 1];
			if (L->Sign && Contains(L->Args, L->Rel->Arity, V))
				break;
		}
	}
...
/*       ----------  */
{
	...
	if (L->Sign) {
		...
		{
			if (L->Args[V] > NewMaxBound)
				NewMaxBound = L->Args[V];
		}
	}
