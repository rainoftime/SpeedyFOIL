...
/*    --------------  */
{
	...
	ForEach(TTN, 1, MaxType)
	{
		...
		if (!Table)
			Table = AllocatePartOrd(MaxConst);
		...
		ClearPartOrd(PO);
		Verbose(3)
			printf("\tFinding maximal consistent set\n");
...
/*    --------------------    */
{
	...
	{
		if (!Compatible[R->Type[FirstArg]][TTN])
			continue;

		ForEach(SecondArg, FirstArg+1, R->Arity)
		{
			if (!Compatible[R->Type[SecondArg]][TTN])
				continue;
...
/*       -----------------  */
{
	...
	while (*TP) {
		...
		if (K == MISSING_DISC || K == OUT_OF_RANGE || L == MISSING_DISC
				|| L == OUT_OF_RANGE)
			continue;
...
/*    -----------  */
{
	...
	if (NArgOrders % 100 == 0) {
		Realloc(ArgOrderList, NArgOrders + 100, ArgOrder);
	}
