...
/*       --------  */
{
	...
	} else if (Name[0] == '#') /* specified to be unordered */
	{
		T->FixedPolarity = true;
		T->Ordered = false;
		T->Name = CopyString(Name + 1);
	...
	if (!strcmp(Name, "continuous")) {
		...
		do {
			while ((FirstTime ? !(FirstTime = false) : (Delim = ReadName(Name)))
					&& Delim == '\n')
				;
...
/*        ------------  */
{
	...
	if (NArgs > MAXARGS)
		MAXARGS = NArgs;
	if (R->PossibleTarget && NArgs > MAXVARS)
		MAXVARS = NArgs;
...
/*    -------------   */
{
	...
	while ((R = ReadRelation())) {
		...
		Verbose(4) {
			...
			for (T = Reln[MaxRel]->Pos; *T; T++) {
				PrintTuple(*T, Reln[MaxRel]->Arity, Reln[MaxRel]->TypeRef,
						Reln[MaxRel]->Neg != Nil);
			}
	...
	while (true) {
		...
		{
			if (Waiting[i]
					&& (Best < 0 || Reln[i]->Arity < Reln[Best]->Arity
...
/*       -------------    */
{
	...
	if (R->Arity != 2 || R->TypeRef[1]->Continuous
			|| R->TypeRef[2]->Continuous) {
		return false;
	}
...
/*     ---------  */
{
	...
	ForEach(Arg, 1, N)
	{
		...
		{
			IX[Arg][Val] =
					Next[Arg][Val] ? Alloc(Next[Arg][Val] + 1, int) : Nil;
			Next[Arg][Val] = 0;
		}
	}
		...
		ForEach(T2, T1+1, MaxType)
		{
			...
			Verbose(2) {
				printf("Types %s and %s %s compatible\n", Type[T1]->Name,
						Type[T2]->Name, Compatible[T1][T2] ? "are" : "are not");
			}
		}
	}
}
...
/*    -----  */
{
	switch (n) {
	case 1:
		printf("Illegal delimiter after %s\n", s1);
		exit(1);

	case 2:
		printf("Something wrong after %s in type %s\n", s1, s2);
		exit(1);

	case 3:
		printf("Undeclared constant %s\n", s1);
		exit(1);

	case 4:
		printf("Constant %s is not of type %s\n", s1, s2);
		exit(1);
	...
	case 6:
		printf("Cannot use CWA for %s (continuous types)\n", s1);
		exit(1);

	case 7:
		printf("Type %s contains duplicate constant %s\n", s2, s1);
		exit(1);
	}
}
...
/*       ----------  */
{
	...
	while ((Case = *T++)) {
		if (Case[F] == Case[S] && Case[F] != OUT_OF_RANGE)
			return false;
	}
