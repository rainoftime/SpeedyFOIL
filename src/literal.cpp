...
//	if ( R == Target && ! AnyPartialOrder ) return;
	...
	if (SDNCONSTRAINT) {
		/* find the BodyV config */
		SDNCommonV = GetSDNConfigBodyV(Target->Name);

		if (SDNCommonV == 255) {
			printf("Error: cannot find SDN config for %s\n", Target->Name);
			return;
		}
	}
...
/*   --------------------  */
{
	...
	{
		if (Compatible[Variable[V]->Type][TNo])
			Sum++;
	}
...
/*       -----------  */
{
	...
	ForEach(i, 0, NLit-1)
	{
		L = NewClause[i];
		if (L->Rel == R
				&& SameArgs(R->Arity, A, Current.MaxVar, L->Args, MaxV,
						i + 1)) {
			return true;
		}
...
/*    ------------  */
{
	...
	ForEach(V, 1, Current.MaxVar-1)
	{
		if (Barred[V] || !Variable[V]->TypeRef->Continuous)
			continue;
		...
		ForEach(W, V+1, Current.MaxVar)
		{
			if (Barred[W] || Variable[W]->Type != Variable[V]->Type)
				continue;
...
Boolean TryArgs(Relation R, int This, int HiVar, int FreeVars, int MaxDepth,
	...
	if (!CountOnly) {
		if (Ticks > TicksCheck) {
			if ((TimeSpent = CPUTime() - StartTime) > MAXTIME) {
				return false;
			} else if (TimeSpent > 0.001 * MAXTIME) {
	...
	if (TryMostGeneral && HiVar + NewFreeVars <= MAXVARS /* enough variables */
		...
		if (AcceptableKey(R, CopyKey) && (R != Target || RecOK || (RecOK =
				RecursiveCallOK(Arg)))) {
			if (CountOnly) {
				R->NTrialArgs++;
			} else if (!Repetitious(R, Arg)) {
				/* Here, This is always >=2, sdn constraint check is not necessary*/
				EvaluateLiteral(R, Arg, R->Bits, &Prune);
				R->NTried++;

				if (Prune && NewFreeVars) {
					Verbose(3)
						printf("\t\t\t\t(pruning subsumed arguments)\n");
					return true;
				}
			}
		}
	...
	for (V = 1; V <= MaxV && BestLitGain < MaxPossibleGain; V++) {
		...
		// add SDN constraint: the first var must be the same  for each relation in the rule body
		if (SDNCONSTRAINT && SDNCommonV > 0 && This == 1
				&& InSDNConfig(R->Name)) {
			if (SDNCommonV != V)
				continue;

			/*
			 printf("Before TryArgs:\n");
			 if (!Target) printf("Target is null.\n");
			 if(!NewClause) printf("NewClause is null.\n");
			 if(Target && NewClause) {
			 PrintComposedLiteral(Target, true, DefaultVars);
			 printf(" R=%s  NLit=%d NDeterminate=%d\n",R->Name, NLit, NDeterminate);
			 ...
			 k = -1 ;
			 for(i = 0; i < NLit + NDeterminate; ++i) { // enumerate each literal in current rule body
			 tmpL = NewClause[i];
			 if ( InSDNConfig(tmpL->Rel->Name) ) {
			 k = tmpL -> Args[1];
			 break;  // match a sdn setting stop searching
			 }
			 }

			 if (k != -1 && k != V ) continue;  // SDN constraint: k should be the same as V
			 */
		}

		Arg[This] = V;

		if (!TryArgs(R, This + 1, Max(HiVar, V), NewFreeVars, NewMaxDepth, Key,
				V <= HiVar, RecOK))
			return false;
	}
