...
/*    ----------  */
{
	...
	{
		if (TypeRef ?
				TypeRef[i]->Continuous : Variable[i]->TypeRef->Continuous) {
			/*  Continuous value  */
...
//                         Variable[A[2]]->Name);
		if (RSign) {
			printf("eq(%s,%s)", Variable[A[1]]->Name,Variable[A[2]]->Name);
		}
		else {
			printf("~eq(%s,%s)", Variable[A[1]]->Name,Variable[A[2]]->Name);
		}
...
//			 ConstName[ThConst]);
		if (RSign) {
			printf("eq(%s,%s)", Variable[A[1]]->Name,ConstName[ThConst]);
		}
		else {
			printf("~eq(%s,%s)", Variable[A[1]]->Name,ConstName[ThConst]);
		}
	...
	{
		if (RSign) {
			printf("gt(%s,%s)", Variable[A[1]]->Name,Variable[A[2]]->Name);
		}
		else {
			printf("~gt(%s,%s))", Variable[A[1]]->Name,Variable[A[2]]->Name);
		}
